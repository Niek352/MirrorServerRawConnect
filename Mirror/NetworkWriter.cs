    ...
    public class NetworkWriter
    {
        ...
        public unsafe bool WriteBytes(byte* ptr, int offset, int size)
        {
            ...
            fixed (byte* destination = &buffer[Position])
            {
                ...
                // 10 mio writes: 775ms
                  Buffer.BlockCopy(buffer, offset, buffer, Position, size);
                // 10 mio writes: 637ms
        ...
        public void Write<T>(T value)
        {
            ...
            {
                Console.WriteLine($"No writer found for {typeof(T)}. This happens either if you are missing a NetworkWriter extension for your custom type, or if weaving failed. Try to reimport a script to weave again.");
            }
            else
            {
